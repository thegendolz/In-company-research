...
void AlphaTree::doAlphaStep(int image_height, int image_width, bool initialize) {
	clock_t step2 = clock();
	diss = dissimilarity.at(dissimilarity.size()-1);
	while (diss.distance >= 0 && diss.distance == alphaLevel) {
		a = 0;
		pointer = findRoot(pixelObjArray[diss.n1][diss.m1].parent);
		//pointer = pixelObjArray[diss.n1][diss.m1].parent;
		b = a;
		a = 0;
		//Path compression inside FindRoot function
		Pixel* pointer2 = nullptr;
		pointer2 = findRoot(pixelObjArray[diss.n][diss.m].parent);
		//pointer2 = pixelObjArray[diss.n][diss.m].parent;
		...
		dissimilarity.erase((dissimilarity.end()-1));
		if (dissimilarity.size() > 0) diss = dissimilarity.at(dissimilarity.size()-1);
		else break;
	}
	printf("--------[Dissies: %d]--------\n", dissimilarity.size());
